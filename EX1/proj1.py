...
def get_hmm(num_hmm):
    ...
    if num_hmm == 1:
        hmm_params = {
            ...
            "transition_mat": np.array(
                [[0.75, 0.25], [0.25, 0.75]]  # p(X_{t+1}=x | X_t=0)
            ),  # p(X_{t+1}=x | X_t=1)
            "emission_mat": np.array([[0.83, 0.17], [0.17, 0.83]]),
        }
        T = 1000
        ...
        hmm_params = {
            "prior": np.array([1, 0]),
            "transition_mat": np.array([[0.999, 0.001], [0.005, 0.995]]),
            "emission_mat": np.array([[0.99, 0.01], [0.01, 0.99]]),
        }
        T = 1000
    elif num_hmm == 3:
        hmm_params = {
            "prior": np.array([1, 0]),
            "transition_mat": np.array([[0.999, 0.001], [0, 1]]),
            "emission_mat": np.array([[0.99, 0.01], [0.01, 0.99]]),
        }
        T = 1000
    ...
    hmm = HMM(
        ...
        transition_mat=hmm_params["transition_mat"],
        emission_mat=hmm_params["emission_mat"],
    )
...
def plot_samples(obs, hidden=None, N_plot=3, fig_name="hmm"):
    if hidden is not None:
        plot_sequence(hidden[:N_plot].T, seq_name=f"hidden", fig_name=fig_name)
...
def Q4_identify_corrupt_data(hmm):
    ...
    avr = np.average(validation_marginal_log_likelihood)
    std = np.std(validation_marginal_log_likelihood)
    find_corrupted = lambda data: data < (avr - 3 * std)
    test_marginal_log_likelihood = hmm.log_likelihood(test_data)
    corrupted = find_corrupted(test_marginal_log_likelihood)
    real_marginal_log_likelihood = test_marginal_log_likelihood[corrupted == False]
    corrupt_marginal_log_likelihood = test_marginal_log_likelihood[corrupted == True]

    # plot histograms
    plt.title("Histogram of marginal log-likelihood")
    mi = np.min(
        [
            corrupt_marginal_log_likelihood.min(),
            real_marginal_log_likelihood.min(),
            validation_marginal_log_likelihood.min(),
        ]
    )
    _, bins, _ = plt.hist(
        validation_marginal_log_likelihood,
        label="validation data",
        bins=np.arange(mi - 10, 0, 4),
    )
        ...
        if invalid_transitions is not None:
            for xt, xtp1 in invalid_transitions:
                ...
                print(
                    np.sum(
                        [
                            ((X_hat[:, t] == 1) & (X_hat[:, t + 1] == 0)).sum()
                            for t in range(hmm.T - 1)
                        ]
                    )
                )
    X_hat = hmm.naive_predict_by_naive_posterior(data_obs)
    print(
        f"Naive prediction accuracy = {accuracy(data_hidden.flatten(), X_hat.flatten())}, confusion mat:"
    )
